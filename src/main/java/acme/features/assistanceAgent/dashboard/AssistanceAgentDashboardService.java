...
@GuiService
public class AssistanceAgentDashboardService extends AbstractGuiService<AssistanceAgent, AssistanceAgentDashboard> {
	//Internal state ---------------------------------------------------------
	...
	public void load() {
		...
		if (!claims.isEmpty()) {
			//claimsResolvedRatio
			Long claimsResueltas = claims.stream().filter(x -> x.getStatus() == TrackingLogStatus.ACCEPTED).count();
			...
			//claimsRejectedRatio
			Long claimsRejected = claims.stream().filter(x -> x.getStatus() == TrackingLogStatus.REJECTED).count();
